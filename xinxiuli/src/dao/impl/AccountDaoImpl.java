...
public class AccountDaoImpl implements AccountDao{
	...
	public boolean updateAccount(Account account, Connection conn) throws Exception {
		boolean flag = false;
		String sql = "update account set account_num=?,account_pass=?,users_ip=?,cart_num=?,order_num=?,coll_goods=? where account_id=?";
		PreparedStatement ps = conn.prepareStatement(sql);
		ps.setString(1, account.getAccount_num());
		...
		ps.setString(7, account.getAccount_id());
		if(ps.executeUpdate()>0) {
			flag = true;
		}
		return flag;
